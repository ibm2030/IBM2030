...
UMD_LCH: FL port map(clk=>sysclk, S=>UMD_LCH_Set, R=>UMD_LCH_Reset, Q=>sUSE_MANUAL_DECODER); -- AC1G4
...
MD_LCH: FL port map(clk=>sysclk, S=>MD_LCH_Set, R=>SW_DSPLY_N, Q=>sMANUAL_DISPLAY); -- AC1G4 - FMD missing invert on Reset input ??
...
MS_LCH: FL port map(clk=>sysclk, S=>MS_LCH_Set, R=>SW_STORE_N, Q=>sMAN_STORE); -- AC1E5
MAN_STORE <= sMAN_STORE;
-- MAN_STORE_PWR <= sMAN_STORE; -- AC1F3 -- Need to delay this a bit
MAN_STORE_DELAY: AR port map(clk=>sysclk, D=>sMAN_STORE, Q=>sMAN_STORE2); -- AC1F3
MAN_STORE2_DELAY: AR port map(clk=>sysclk, D=>sMAN_STORE2, Q=>MAN_STORE_PWR); -- AC1F3
...
ALLOW_WRITE_LCH: FL port map(clk=>sysclk, S=>AW_LCH_Set, R=>AW_LCH_Reset, Q=>sALLOW_WRITE); -- AA1J2,AA1F6,AA1H3
ALLOW_WRITE <= sALLOW_WRITE;	
DELAY_ALLOW_WR : AR port map (clk=>sysclk, D=>sALLOW_WRITE, Q=>sALLOW_WR); -- AA1H2,AA1J7
...
MW_LCH: FL port map(clk=>sysclk, S=>MW_LCH_Set, R=>MW_LCH_Reset, Q=>MAN_WR_CALL); -- AC1J2,AC1F4,AC1H5

-- MAN RD INLK
sMAN_STOR_OR_DSPLY_N <= not sMAN_STOR_OR_DSPLY;
MAN_RD_INLK_FL: FL port map(clk=>sysclk, S=>MAN_RD_CALL_LCH, R=>sMAN_STOR_OR_DSPLY_N, Q=>MAN_RD_INLK); -- AC1F4
-- MAN RD CALL
MRC_LCH_Set <= sSTG_MEM_SEL and not MAN_RD_INLK and sMAN_STOR_OR_DSPLY;
MRC_LCH_Reset <= not sMAN_STOR_OR_DSPLY or READ_ECHO;
MAN_RD_CALL_FL: FL port map(clk=>sysclk, S=>MRC_LCH_Set, R=>MRC_LCH_Reset, Q=>MAN_RD_CALL_LCH); -- AC1J2,AC1E2
...
SR_LCH: FL port map(clk=>sysclk, S=>SR_LCH_Set, R=>SR_LCH_Reset, Q=>sSTORE_R); -- 06C
STORE_R <= sSTORE_R;
