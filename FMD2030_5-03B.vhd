...
UWRAP_LCH: PHR port map(clk=>sysclk, D=>WRAP_TRUE,L=>CHECK_U_WRAP,R=>UWRAP_LCH_Reset,Q=>U_WRAP_CPU); -- AB2M4
IWRAP_LCH: PHR port map(clk=>sysclk, D=>WRAP_TRUE,L=>CHECK_I_WRAP,R=>RECYCLE_RST,Q=>sI_WRAPPED_CPU); -- AB2M4
I_WRAPPED_CPU <= sI_WRAPPED_CPU;
UMPX_LCH: PHR port map(clk=>sysclk, D=>CARRY_OUT,L=>CHECK_MPX_WRAP,R=>RECYCLE_RST,Q=>sU_WRAPPED_MPX); -- AB2M4 ?? Doesn't have reset in FMD - causes Diag failure
U_WRAPPED_MPX <= sU_WRAPPED_MPX;
WBUFF_LCH: PHR port map(clk=>sysclk, D=>sI_WRAPPED_CPU,L=>STORE_WRAP,R=>'0',Q=>WRAP_BUFF); -- AB2M4 ?? *not* sI_WRAPPED_CPU ??
...
MWR_LCH: FL port map(clk=>sysclk, S=>MWR_LCH_Set, R=>MWR_LCH_Reset, Q=>sMEM_WRAP_REQ);
