...
signal	PREFIX_SET,PREFIX_RESET : STD_LOGIC;
signal  PREFIX : STD_LOGIC := '0';
signal	BLOCK_SHIFT_SET : STD_LOGIC;
signal  BLOCK_SHIFT : STD_LOGIC := '0';
signal	sWRITE_LCH : STD_LOGIC := '0';
signal	UPPER_CASE_DECODE, LOWER_CASE_DECODE : STD_LOGIC;
signal	sRD_SHARE_REQ_LCH : STD_LOGIC := '0';
...
signal	PRT_IN_UC_SET, PRT_IN_UC_RESET : STD_LOGIC;
signal  PRT_IN_UC : STD_LOGIC := '0';
...
XLATE_UC_FL: FL port map (clk=>sysclk, S=>XLATE_UC_SET, R=>XLATE_UC_RESET, Q=>XLATE_UC); -- ?????
...
RD_SHARE_REQ_FL: FL port map(clk=>sysclk, S=>RD_SHARE_REQ_SET, R=>RD_SHARE_REQ_RESET, Q=>sRD_SHARE_REQ_LCH); -- AC3F5 AC3C7
...
PREFIX_FL: FL port map(clk=>sysclk, S=>PREFIX_SET,R=>Y_TIME,Q=>PREFIX); -- AC3F2 AC3G5
-- Block Shift prevents the shift mechanism from being triggered
BLOCK_SHIFT_SET <= PREFIX and X_TIME;
BLOCK_SHIFT_FL: FL port map(clk=>sysclk, S=>BLOCK_SHIFT_SET,R=>W_TIME,Q=>BLOCK_SHIFT); -- AC3F2 AC3C6
...
PRINT_IN_UC_FL: FL port map(clk=>sysclk, S=>PRT_IN_UC_SET,R=>PRT_IN_UC_RESET,Q=>PRT_IN_UC); -- ?????
WRITE_UC <= PRT_IN_UC;
...
WRITE_FL : FL port map(clk=>sysclk, S=>WRITE_SET,R=>WRITE_RESET,Q=>sWRITE_LCH); -- AC2J5 AC2H6
