...
attribute mark_debug of PROC_STOP_LOOP_ACTIVE: signal is "true";
attribute mark_debug of SEL_ROS_REQ: signal is "true";
attribute mark_debug of MPX_SHARE_REQ: signal is "true";
attribute mark_debug of LOAD_REQ: signal is "true";
attribute mark_debug of sLOAD_KEY_SW: signal is "true";
attribute mark_debug of SYSTEM_RESET_SW: signal is "true";
attribute mark_debug of MACH_RST_3, sHARD_STOP_LCH: signal is "true";
attribute mark_debug of GT_J_REG_TO_A_BUS: signal is "true";
attribute mark_debug of CF_STOP: signal is "true";
attribute mark_debug of HS_MACH_CHK: signal is "true";
attribute keep of HS_MACH_CHK: signal is "true";
attribute mark_debug of sEARLY_ROAR_STOP: signal is "true";
attribute mark_debug of HS_ALU_CHK: signal is "true";
attribute keep of HS_ALU_CHK: signal is "true";
attribute mark_debug of HS_DIAG: signal is "true";
attribute keep of HS_DIAG: signal is "true";
attribute mark_debug of HS_MATCH: signal is "true";
attribute keep of HS_MATCH: signal is "true";
attribute mark_debug of HS_INSTR: signal is "true";
attribute keep of HS_INSTR: signal is "true";

BEGIN
-- Fig 5-03C
-- STT RST INLK
SRI_LCH: FL port map(clk=>sysclk, R=>sSTART_SW_RST,S=>SW_START,Q=>STT_RST_INLK); -- AC1G7 - Note inputs reversed to make inverted output
-- STT RST
SSR_LCH_Set <= ALLOW_MAN_OPER and STT_RST_INLK and not SW_START;
SSR_LCH_Reset <= T2 or MACH_RST_SW;
SSR_LCH: FL port map(clk=>sysclk, S=>SSR_LCH_Set,R=>SSR_LCH_Reset,Q=>sSTART_SW_RST); -- AC1G7
START_SW_RST <= sSTART_SW_RST;
-- CLK STT CTRL
CSC_LCH_Set <= sCLOCK_RST or sE_CY_STOP_SAMPLE;
CSC_LCH: FL port map(clk=>sysclk, S=>CSC_LCH_Set,R=>sSTART_SW_RST,Q=>CLK_STT_CTRL); -- AC1F5
-- E CY STOP SAMPLE
...
ECS_LCH: FL port map(clk=>sysclk, S=>ECS_LCH_Set, R=>ECS_LCH_Reset, Q=>sE_CY_STOP_SAMPLE); -- AC1F7
E_CY_STOP_SAMPLE <= sE_CY_STOP_SAMPLE;	
-- LOAD KEY INLK
LKI_LCH_Set <= (not SW_LOAD and MACH_RST_3) or LOAD_KEY;
LKI_LCH: FL port map(clk=>sysclk, R=>LKI_LCH_Set, S=>SW_LOAD, Q=>sLOAD_KEY_INLK); -- AC1F7 - Note inputs reversed to make inverted output
LOAD_KEY_INLK <= sLOAD_KEY_INLK;
-- LOAD KEY
LK_LCH_Set <= not sLOAD_KEY_SW and sLOAD_KEY_INLK;
LK_LCH_Reset <= T4 or sCLOCK_RST;
LK_LCH: FL port map(clk=>sysclk, S=>LK_LCH_Set, R=>LK_LCH_Reset, Q=>LOAD_KEY); -- AC1F7
...
SI_LCH: FL port map(clk=>sysclk, S=>SI_LCH_Set, R=>SI_LCH_Reset, Q=>SET_IC_INLK); -- AC1G7
-- SET IC
SIA_LCH_Set <= ALLOW_MAN_OPER and not SET_IC_INLK and SW_SET_IC;
SIA_LCH_Reset <= T2 or MACH_RST_SW;
SIA_LCH: FL port map(clk=>sysclk, S=>SIA_LCH_Set, R=>SIA_LCH_Reset, Q=>sSET_IC_ALLOWED); -- AC1G7
...
PS_LCH: FL port map(clk=>sysclk, S=>PS_LCH_Set, R=>PS_LCH_Reset, Q=>PROCESS_STOP); -- AC1E5
...
CFS_LCH: FL port map(clk=>sysclk, S=>CF100T4, R=>CFS_LCH_Reset, Q=>CF_STOP); -- AC1D5
sROS_SCAN <= SW_SCAN;
ROS_SCAN <= sROS_SCAN;
...
LOAD_REQ_FL: FL port map(clk=>sysclk, S=>LOAD_KEY, R=>sRST_SEL_CHNL_DIAG_LCHS, Q=>sLOAD_IND); -- AC1E5
...
CS_LCH: FL port map(clk=>sysclk, S=>CS_LCH_Set, R=>CS_LCH_Reset, Q=>sCLOCK_START_LCH); -- AC1K6
...
N2E_LCH: FL port map(clk=>sysclk, S=>N2E_LCH_Set, R=>N2E_LCH_Reset, Q=>sN2ND_ERROR_STOP); -- AB3F4
...
PO_LCH: FL port map(clk=>sysclk, S=>PO_LCH_Set, R=>MACH_START_RST, Q=>PWR_OFF); -- AC1F4
-- HARD STOP
...
HS_LCH: FL port map(clk=>sysclk, S=>HS_LCH_Set, R=>MACH_START_RST, Q=>sHARD_STOP_LCH); -- AB3H6
