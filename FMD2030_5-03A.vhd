...
SUPPR_MALF_TRAP_LCH: PHR port map(clk=>sysclk, D=>R_REG_5_N,L=>SUPPR_MACH_TRAP_L,R=>RECYCLE_RST,Q=>sSUPPR_MACH_CHK_TRAP); -- AB3D2,AB3J2
...
ANY_PRIORITY: PH port map(clk=>sysclk, D=>sANY_PRIORITY_PULSE_2, L=>T1, Q=>sANY_PRIORITY_LCH); -- AB3D7,AB3J2
ANY_PRIORITY_LCH <= sANY_PRIORITY_LCH;
S1_DLYD: PH port map(clk=>sysclk, D=>S_REG_1_BIT, L=>T1, Q=>S_REG_1_DLYD); -- AB3J2
WX_SABC: PH port map(clk=>sysclk, D=>sGT_SWS_TO_WX_LCH, L=>T1, Q=>sGT_SW_TO_WX_LCH); -- AB3J2
GT_SW_TO_WX_LCH <= sGT_SW_TO_WX_LCH;
CD0101 <= '1' when SALS_CDREG="0101" else '0';
PRIOR_RST_Latch <= T4 or MACH_RST_SW;
PRIOR_RST_CTRL_PH: PHR port map(clk=>sysclk, D=>CD0101,L=>PRIOR_RST_Latch,R=>sANY_PRIORITY_PULSE,Q=>PRIOR_RST_CTRL); -- AB3J2
MEMP_LCH_Set <= sDATA_READY and ALLOW_PROTECT and PROT_LOC_CPU_OR_MPX;
MEMP_LCH_Reset <= READ_CALL or RECYCLE_RST;
STG_PROT_REQ: FL port map(clk=>sysclk, S=>MEMP_LCH_Set,R=>MEMP_LCH_Reset,Q=>sMEM_PROTECT_REQ); -- AA1K7
...
PRIORITY: FL port map(clk=>sysclk, S=>PRI_LCH_Set,R=>PRI_LCH_Reset,Q=>PRIORITY_LCH); -- AB3J4,AB3L4
...
PRISTK_LCH: PHV port map(clk => sysclk, D => PRIORITY_STACK_IN, L=>PRISTK_LCH_Latch, Q=>PRIORITY_STACK_OUT);
...
SRP_LCH: FL port map(clk=>sysclk, S=>MACH_RST_SW,R=>T4,Q=>sSYS_RST_PRIORITY_LCH); -- AB3L3
