...
RD_CLK_INLK: FL port map(clk=>sysclk, S=>RD_CLK_INLK_SET, R=>RD_INLK_RST, Q=>sRD_CLK_INLK_LCH); -- AC3E3, AC3F2
...
n1050_RST : FL port map(clk=>sysclk, S=>n1050_RST_SET, R=>n1050_RST_RESET, Q=>s1050_RST_LCH); -- AC2K5 AC2E2
...
HOME_RDR_STT_FL: FL port map(clk=>sysclk, S=>SET_HOME_RDR_STT, R=>TA_REG_RST, Q=>sHOME_RDR_START_LCH); -- AC2H3 AC2K4
...
PROCEED_FL: FL port map(clk=>sysclk, S=>SET_PROCEED, R=>TA_REG_RST, Q=>sPROCEED_LCH); -- AC2D6 AC2K7
...
MICRO_SHARE_FL: FL port map(clk=>sysclk, S=>SET_MICRO_SHARE, R=>TA_REG_RST, Q=>sMICRO_SHARE_LCH); -- AC2H3 AC2K4
MICRO_SHARE_LCH <= sMICRO_SHARE_LCH;

SET_RDR_2 <= TAGS_OUT_BUS(1) and sTA_REG_SET;
RDR_2_FL: FL port map(clk=>sysclk, S=>SET_RDR_2, R=>TA_REG_RST, Q=>sRDR_ON_LCH); -- AC2H3 AC2K4
...
MS1000: SS port map(clk50M,5000000,MS1000_IN, sCR_LF); -- AC2L2 AC2D6 1s single-shot : 100ms (5000000) is enough
CR_LF <= sCR_LF;
...
SEL_O_DET_FL: FL port map(clk=>sysclk, S=>SET_SEL_O_DET, R=>RESET_SEL_O_DET, Q=>SEL_O_DET); -- AC3E6

SET_SEL_O_DLY <= T3 and SEL_O_DET and not sn1050_OP_IN;
RESET_SEL_O_DLY <= sRST_ATTACH or not CPU_SEL_O_OR_SEL_IN;
SEL_O_DLY_FL: FL port map(clk=>sysclk, S=>SET_SEL_O_DLY, R=>RESET_SEL_O_DLY, Q=>SEL_O_DLY); -- AC3E6
...
n1050_OP_IN_FL: FL port map(clk=>sysclk, S=>SET_1050_OP_IN, R=>RESET_1050_OP_IN, Q=>sn1050_OP_IN);
n1050_OP_IN <= sn1050_OP_IN;

SET_1050_EXIT_SHARE_REQ <= MPX_OPN_LCH_GT and not CK_SAL_P_BIT; -- AC3C7
n1050_EXIT_SHARE_REQ_FL : FL port map(clk=>sysclk, S=>SET_1050_EXIT_SHARE_REQ, R=>T1, Q=>n1050_EXIT_SHARE_REQ); -- AC3C6 AC3E4
...
PREPARE_TO_SHARE_FL: FL port map(clk=>sysclk, S=>SET_PREPARE_TO_SHARE, R=>RESET_PREPARE_TO_SHARE, Q=>PREPARE_TO_SHARE); -- AC3E6
...
ATTN_INTLK_FL: FL port map(clk=>sysclk, S=>SET_ATTN_INTLK, R=>RESET_ATTN_INTLK, Q=>ATTN_INTLK); -- AC3C6 AC3D6 - ?? Not sure about this
